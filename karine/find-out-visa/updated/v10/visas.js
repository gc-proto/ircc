...
$(document).on("wb-ready.wb", function (event) {
    ...
    function handleNextClick() {
        ...
        if (currentQuestion.id === "question-passport_code" && passportCodeSelectionParent.classList.contains("hidden")) {
            ...
            */
            ...
            btnReset.classList.toggle("hidden", nextQuestionId.includes("question"));
            btnChange.classList.toggle("hidden", nextQuestionId.includes("question"));
            btnNext.classList.toggle("hidden", nextQuestionId.includes("result"));

            currentQuestion.classList.add("hidden");
            // nextQuestion.classList.remove("hidden");


            toolContainer.classList.toggle("results", nextQuestionId.includes("result"));
            if (nextQuestionId.includes("result")) {
                let resultAjax = document.createElement('div');
                resultAjax.setAttribute('id', nextQuestionId);
                resultAjax.setAttribute('tabindex', "-1");
                resultAjax.setAttribute('class', "result ajax");
                resultAjax.setAttribute('data-wb-ajax', `{"type": "append", "url": "results/${nextQuestionId.split('result-')[1]}.html", "nocache": "nocache", "trigger": true}`);
                document.querySelector('.results-container').append(resultAjax);
                $( resultAjax ).trigger( "wb-init.wb-data-ajax" );
    ...
    function handlePreviousClick(changeAnswer, reset) {
        ...
        document.querySelector('.result.ajax').remove();
        if (changeAnswer) {
